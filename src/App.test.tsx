...
describe("Cunxun demo app", () => {
  ...
  it("opens a village route navigation surface for the selected NPC", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "出发导航" }));
  ...
  it("gives visitors a clear first action and current NPC availability", () => {
    ...
    expect(screen.getByRole("button", { name: "导航去找当前人物" })).toBeInTheDocument();
  ...
  it("surfaces a featured strip with the three soonest-available NPCs and a prototype watermark", () => {
    ...
    expect(screen.getByText(/AI 分身 Demo · 种子 3/)).toBeInTheDocument();
    expect(screen.getByTestId("density-watermark")).toHaveTextContent("原型预览 · 非真实人物");
  ...
  it("switches into a focused route mode with walking time and an exit", () => {
    ...
    expect(screen.getByText("100 人规模预览")).toBeInTheDocument();
  });
  ...
  it("starts in a clear overview state and locates visitors from the lower-right map control", () => {
    ...
    expect(mapStatus).toHaveTextContent("280 米内");
  ...
  it("adds a separate village place layer that can open reference locations", () => {
    render(<App />);

    expect(screen.getByLabelText("村庄地点")).toBeInTheDocument();
    expect(screen.getAllByTestId("village-place-marker").length).toBeGreaterThanOrEqual(4);
    expect(screen.getByRole("button", { name: "四坪村口，地点，入口" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "随喜书屋，地点，公共空间" })).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "放大地图" }));
    fireEvent.click(screen.getByRole("button", { name: "放大地图" }));

    expect(screen.getByRole("button", { name: "小毛驴四坪农园，地点，土地线索" })).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "小毛驴四坪农园，地点，土地线索" }));

    expect(screen.getByRole("region", { name: "小毛驴四坪农园地点说明" })).toBeInTheDocument();
    expect(screen.getByText("地点层 · 待现场校准")).toBeInTheDocument();
    expect(screen.getByText(/正式上线前需要现场 GPS 校准/)).toBeInTheDocument();
    expect(screen.queryByLabelText("小满的地图信息")).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "查看附近 NPC" }));

    expect(screen.getByText("显示 49 / 100 · 四坪")).toBeInTheDocument();
  });

  it("focuses high zoom on NPCs near my current location", () => {
    render(<App />);
    ...
    expect(screen.getByText("160%")).toBeInTheDocument();
    expect(screen.getByText("附近 NPC")).toBeInTheDocument();
    expect(screen.getByText("显示 10 / 100 · 280 米内 · 四坪")).toBeInTheDocument();
    expect(screen.getByTestId("density-preview-note")).toBeInTheDocument();
    expect(screen.queryAllByTestId("density-person-dot")).toHaveLength(0);
    fireEvent.click(screen.getByRole("button", { name: "显示预览点" }));
    expect(screen.getAllByTestId("density-person-dot")).toHaveLength(10);
    expect(screen.getByRole("button", { name: /^小满，/ })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /^阿楷，/ })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /^阿岚，/ })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "回到本村主览" })).toBeInTheDocument();
